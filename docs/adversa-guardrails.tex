...
\begin{tikzpicture}[
    ...
    \node (triage) [layer=blue!8, draw=blue!70!black] {
        \textbf{Phase 1 — Disk Agent + Memory Agent} \small(\texttt{blue\_agent.py} + \texttt{memory\_agent.py}) \\[2pt]
        \footnotesize
        Disk Agent Pass 1: $\sim$25 deterministic SIFT commands · corpus-calibrated log-odds scoring \\[1pt]
        Disk Agent Pass 2: 75-call Claude agentic loop · extracts IOCs · scores MITRE techniques \\[1pt]
        Memory Agent: Volatility 3 parallel path · process injection · VAD anomalies · credential dumps
    };
    ...
    \node (risk) [callout, right=2.2cm of triage] {
        ...
        \textit{nfury: 19 techniques investigated — Auditor confirmed 15, refuted 4}
    };
    ...
    \draw[->, thick, draw=orange!70!black]
        ...
        node[right, font=\tiny\itshape] {5 rounds}
        (auditor.east);
    ...
    \node (result) [callout, right=2.2cm of verdicts] {
        \textbf{Live Result — nfury} \\[3pt]
        \scriptsize 19 techniques investigated \\
        15 confirmed · 4 refuted \\
        Every finding cited to physical artifact \\
