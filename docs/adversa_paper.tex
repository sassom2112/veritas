...
Python backdoor, a distinct tool family from the httppump variants
seen on nfury and tdungan. Spinlock established a TLS connection to
an external C2 at \texttt{199.73.28.114:443} presenting a self-signed
certificate with \texttt{CN=199.73.28.114}---the only confirmed
external command-and-control node in the campaign. \texttt{PSEXESVC.EXE}
was confirmed on disk (T1569.002), and Volatility~3
\texttt{windows.hashdump} extracted credentials from the
\texttt{vibranium} account. T1055 was confirmed in memory via
anonymous \texttt{PAGE\_EXECUTE\_READWRITE} VAD regions.

Refuted (4): four memory-only signals with no disk corroboration---the
identical refutation pattern seen on both nfury and tdungan. The
consistency across three independent host investigations is not
coincidental: it reflects the Auditor's discrimination criterion
operating uniformly.

Total runtime: $\approx$880\,s ($\approx$15\,min); total cost: $\approx$\$14.
