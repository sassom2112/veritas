...
\begin{tikzpicture}[
    ...
    \node (L2) [layer=blue!8, draw=blue!70!black, below=of L1] {
        ...
        \footnotesize
        \textbf{Phase 1a} — Disk Agent (\texttt{blue\_agent.py}):
            Pass 1 deterministic sweep ($<$60 s)
            $\rightarrow$ ASL scoring
            $\rightarrow$ Pass 2 agentic loop (25 calls, event logs / prefetch / shellbags / LSA) \\[1pt]
        \textbf{Phase 1b} — Memory Agent (\texttt{memory\_agent.py}):
            Volatility 3 parallel path · process injection / VAD anomalies / credential dumps \\[2pt]
        \textbf{Phase 2} — Forensic Auditor (\texttt{auditor\_agent.py}):
            \texttt{asyncio.gather} · 5 rounds $\times$ 3 tools per technique ·
